...
Page({
  ...
  onLoad: function(options) {
    ...
    if (prevUrl == 'pages/goods_detail/goods_detail' || prevUrl == 'pages/address/insert/insert') {
      ...
      util.myWxRequest(app.globalData.getOrdersByPayUUID, {
        orderUUId: uuId
      }, function(res) {
        that.setData({
    ...
    if (app.globalData.buyGoods.addressId) {
      ...
      util.myWxRequest(app.globalData.getAddrByIdUrl, {
        id: app.globalData.buyGoods.addressId
      }, function(res) {
        that.setData({
  ...
  commitOrder: function() {
    ...
    // 判断地址
    if (util.isEmpty(that.data.address.id)) {
      wx.showModal({
        title: '提示',
        content: '您还没有默认地址，请填写地址',
        success: function(res) {
...
function frank(that) {
  ...
  if (orders.away) {
    ...
    // 添加订单，获取直接购买商品总价格
    util.kumyWxRequest(app.globalData.addOrder, {
      type: 0,
      ...
      util.myWxRequest(app.globalData.getOrdersUUIDByPayUUID, {
        ...
        //根据地址来计算邮费
        util.myWxRequest(app.globalData.carriageUrl, {
          num: Xnum,
          addrCity: addrCity,
          goodsId: Ggid,
          uuid: Uuid
    ...
    util.myWxRequest(app.globalData.getAddrByDefaultUrl, {
      ...
      let oname = orders.goods;
      for (let k = 0; k < oname.length; k++) {
        gnames += oname[k].goodsName + ',';
      }
      let addrCity = '';
