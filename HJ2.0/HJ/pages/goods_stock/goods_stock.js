...
Page({
  ...
  onLoad: function (options) {
    ...
    // 获取库存商品
    utils.myWxRequest(app.globalData.getCountUrl, { dealerCode: options.dealerCode, page: page, pageSize: pageSize }, function (res) {
      console.log(res.data.data.PageInfo.list);
      that.setData({
        stock: res.data.data.PageInfo.list
      });
    });
  },
  ...
  stockDetail: function (e) {
    ...
    // 跳转
    wx.navigateTo({
      url: '/pages/stock_detail/stock_detail?gid=' + gid + '&dealerCode=' + this.data.dealerCode,
    })
  },
})